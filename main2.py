        ...
        service_context = ServiceContext.from_defaults(
            llm_predictor=LLMPredictor(
                llm=ChatOpenAI(
                    ...
                    max_tokens=512,
                    # temperature=0,
                    # model_name="ft:gpt-3.5-turbo-1106:personal::9hWLayjR",
                    # max_tokens=512
                )
            )
        )
...
def generate_response(
    ...
    if user_status_code == "USER__RESERVATION_NEW":
        ...
        if is_valid_date(bot_response):
            ...
            RESERVATION_RECEPTION_STAY = (
                f"{reserve['start']} {MESSAGES['reservation_reception_stay']}"
            )
    ...
    if user_status_code == "USER__RESERVATION_NEW_START":
        ...
        if is_single_digit_number(bot_response):
            ...
            RESERVATION_RECEPTION_NUMBER = (
                textwrap.dedent(f"宿泊数は {reserve['stay']} {MESSAGES['reservation_reception_number']}").strip()
            )
    ...
    if user_status_code == "USER__RESERVATION_NEW_STAY":
        ...
        if is_single_digit_number(bot_response):
            ...
            RESERVATION_RECEPTION_SMOKING = textwrap.dedent(f"""
                利用者人数は {reserve['number']} {MESSAGES['reservation_reception_smoking']}
            """).strip()
    ...
    if user_status_code == "USER__RESERVATION_NEW_NUMBER":
        ...
        reserve["smoking"] = bot_response
        if reserve["smoking"] == "禁煙": 
            if is_valid_smoking(reserve["smoking"]):
                RESERVATION_RECEPTION_ROOM = textwrap.dedent(f"""
                    禁煙か喫煙かは {reserve['smoking']}ですね。 {MESSAGES['reservation_reception_room']}
                """).strip()
            else:
        ...
        elif reserve["smoking"] == "喫煙":
            if is_valid_smoking(bot_response):
                RESERVATION_RECEPTION_ROOM = textwrap.dedent(f"""
                禁煙か喫煙かは {reserve['smoking']}ですね。 {MESSAGES['reservation_reception_room_smoke']}
                """).strip()
            else:
                RESERVATION_RECEPTION_ROOM = MESSAGES["reservation_reception_room_error"]
                return str(RESERVATION_RECEPTION_ROOM), user_status_code
